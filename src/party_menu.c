...
static void CB2_ShowPokemonSummaryScreen(void)
{
    if (gPartyMenu.menuType == PARTY_MENU_TYPE_IN_BATTLE)
    {
        ...
        ShowPokemonSummaryScreen(SUMMARY_MODE_LOCK_MOVES, gPlayerParty, gPartyMenu.slotId, gPlayerPartyCount - 1, CB2_ReturnToPartyMenuFromNickname);
    }
    else
    {
        ShowPokemonSummaryScreen(SUMMARY_MODE_NORMAL, gPlayerParty, gPartyMenu.slotId, gPlayerPartyCount - 1, CB2_ReturnToPartyMenuFromNickname);
    }
}
...
static void Task_LearnedMove(u8 taskId)
{
    ...
    if (move[1] == 0)
    {
        ...
        // if (item < ITEM_HM01_CUT) /* these 2 lines cause TMs to be used up */
        //     RemoveBagItem(item, 1);
    }
    GetMonNickname(mon, gStringVar1);
